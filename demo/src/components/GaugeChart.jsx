...
export default function GaugeChart({ value = 0, size = 200 }) {
  ...
  const color = v >= 0.75 ? '#22C55E' : v >= 0.35 ? '#F59E0B' : '#EF4444';
  const label = v >= 0.75 ? 'RỦI RO THẤP' : v >= 0.35 ? 'RỦI RO TRUNG BÌNH' : 'RỦI RO CAO';
  const la = v > 0.5 ? 1 : 0;
  ...
  return (
    <svg width={size} height={size * 0.65} viewBox={`0 0 ${size} ${size*0.65}`} style={{display:'block',margin:'0 auto'}}>
      {/* Zone backgrounds */}
      <path d={arc(0, 0.35)}  fill="none" stroke="rgba(239,68,68,0.14)"  strokeWidth={sw} />
      ...
      <text x={cx} y={cy + r*0.1} textAnchor="middle" fill="#444"
        fontSize={size*0.058} fontFamily="'DM Sans',sans-serif">/ 100</text>
      <text x={cx} y={cy + r*0.27} textAnchor="middle" fill={color}
        fontSize={size*0.062} fontWeight="700" fontFamily="'DM Sans',sans-serif">
        {label}
      </text>
      {/* Scale */}
      <text x={sx-2} y={cy+size*0.07} textAnchor="middle" fill="#333" fontSize={size*0.052} fontFamily="'JetBrains Mono',monospace">0</text>
      <text x={ex+2} y={cy+size*0.07} textAnchor="middle" fill="#333" fontSize={size*0.052} fontFamily="'JetBrains Mono',monospace">100</text>
    </svg>
  );
}
